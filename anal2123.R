...
# Preallocate the result_df data frame with the correct dimensions
result_df <- data.frame(
  Date = character(length(AWEI_images)),
  ImageName = character(length(AWEI_images)),
  stringsAsFactors = FALSE
)

# Iterate through each image file.
for (k in 1:length(AWEI_images)) {
  img <- AWEI_images[k]
  date_str <- substr(basename(img), 7, 14)
  formatted_date <- paste(substr(date_str, 1, 4), substr(date_str, 5, 6), substr(date_str, 7, 8), sep = "-")
  
  # Create a row for the result dataframe.
  row <- data.frame(Date = formatted_date, ImageName = basename(img), stringsAsFactors = FALSE)
  
  # Calculate pixel counts and areas for all reservoirs
  reservoir_pixels <- sapply(1:nrow(AOI_b), function(i) CountPixels(img, AOI_b[AOI_b$id == i, ]))
  col_px <- paste("Reservoir", 1:nrow(AOI_b), "px", sep = "_")
  col_area <- paste("Reservoir", 1:nrow(AOI_b), "area(Km2)", sep = "_")
  
  row[col_px] <- reservoir_pixels
  row[col_area] <- reservoir_pixels * 1e-6 # Convert to square kilometers.
  
  # Add the row to the result dataframe.
  result_df[k, ] <- row
  
  # Create progression bar.
  progress <- round((k / length(AWEI_images)) * 100, 2)
...
write.csv(result_df, file = "./awei_result_df.csv", row.names = FALSE)
